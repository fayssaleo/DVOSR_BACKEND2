...
class VoyageController extends Controller
{
        ...
        if($request->id==""){
            ...
            foreach ($request->crane_voyages as $crane_voyage){
                if($crane_voyage["id"]==""){
                    ...
                    $returnedValue=$this->crane_vouyages_validatorAndSaver($crane_voyage,$voyage->id,$voyage->id,$action->id);
                    if($returnedValue['IsReturnErrorRespone']){
                        return [
                            "payload" => $returnedValue['payload'],
                            "status" => $returnedValue['status']
    ...
    public function deleteOtherDelay(Request $request){
        ...
        if(!$otherDelay){
            ...
            $action->save();
            $this->saveActionDetail($action->id,"other_delays","Empty","Empty",$otherDelay->id,"Empty");
            $otherDelay->delete();
